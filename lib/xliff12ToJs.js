...
function createUnits(childElements) {
    return childElements.reduce((parent, child) => {
        ...
        const children = child.elements.filter(
            (e) => e.name === "trans-unit" || e.name === "group"
        );
        if (children.length) {
...
function createTransUnitTag(transUnit) {
    const jsUnit = transUnit.elements.reduce(
        (unit, element) => {
            switch (element.name) {
                case "source":
                case "target":
                case "note":
                    unit[element.name] = extractValue(
                        element.elements,
                        ElementTypes12
                    );
                    break;
...
export default function xliff12ToJs(str, options, cb) {
    if (!cb && options === undefined) {
        return new Promise((resolve, reject) =>
            xliff12ToJsClb(str, options, (err, ret) =>
